...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" lang="en">
    ...
    <body>
        ...
        // Connect and select:
//        $dbc = mysql_connect('localhost', 'learnphpuser', 'supersecret');
//        mysql_select_db('myblog', $dbc);
        ...
        if ( $r = mysql_query($query, $dbc) ) { // Run the query.
            ...
            while ( $row = mysql_fetch_array($r) ) {
                $entry_with_breaks = nl2br($row['entry']);
                print "<p><h3>{$row['title']}</h3>
                        {$entry_with_breaks}<br />
                        <a href=\"Pursue12-2edit_entry.php?id={$row['entry_id']}\">Edit</a>
                        <a href=\"Pursue12-2delete_entry.php?id={$row['entry_id']}\">Delete</a>
                    </p><hr />\n";
            }
        }
